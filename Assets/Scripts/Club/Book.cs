...
public class Book : MonoBehaviour
{
    ...
    public List<PropInfo> bookPages;
    ...
    public BookPage LeftObj;
    public BookPage RightObj;
    ...
    public BookPage LeftNextObj;
    public Image Right;
    public Image RightNext;
    public BookPage RightNextObj;
    ...
    {
        if (currentPage >= bookPages.Count-1 || bookPages.Count <= 1) return;
        pageDragging = true;
        mode = FlipMode.RightToLeft;
        f = point;

        LeftNextObj.SetPageInfo((currentPage <= bookPages.Count - 1) ? bookPages[currentPage] : new PropInfo());
        RightNextObj.SetPageInfo((currentPage <= bookPages.Count - 2) ? bookPages[currentPage + 1] : new PropInfo());
        ...
        Left.sprite = Right_Sprite;
        Left.transform.SetAsFirstSibling();

        Right.gameObject.SetActive(true);
        Right.transform.position = RightNext.transform.position;
        Right.transform.eulerAngles = new Vector3(0, 0, 0);
        Right.sprite = Left_Sprite;

        RightObj.SetPageInfo((currentPage < bookPages.Count - 2) ? bookPages[currentPage+2] : new PropInfo());
        RightNext.sprite = (currentPage < bookPages.Count - 2) ? Right_Sprite : background;
    ...
    public void DragLeftPageToPoint(Vector3 point)
    {
        ...
        LeftNextObj.SetPageInfo((currentPage >= 2) ? bookPages[currentPage - 2] : new PropInfo());
        RightNextObj.SetPageInfo((currentPage >= 2) ? bookPages[currentPage - 1] : new PropInfo());
        ...
        LeftObj.SetPageInfo((currentPage >= 3) ? bookPages[currentPage - 3] : new PropInfo());

        LeftNext.sprite = (currentPage > 2) ? Left_Sprite : background;
    ...
    void Flip()
    {
        ...
        {
            currentPage += 2;
            LeftObj.SetPageInfo((currentPage <= bookPages.Count) ? bookPages[currentPage-1] : new PropInfo());
        }
        else
        {
            currentPage -= 2;
            RightObj.SetPageInfo((currentPage >= 0) ? bookPages[currentPage] : new PropInfo());
        }
