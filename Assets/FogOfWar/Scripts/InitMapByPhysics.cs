    ...
    public class InitMapByPhysics : MonoBehaviour
    {
        ...
        public void PhysicsCheck()
        {
            ...
            for (int i = 0; i < mapData.GetLength(0); i++)
            {
                ...
                {
                    if (Physics.CheckBox(fowManager.GetV3(new int[] { i, j }), new Vector3(fowManager.MapTileSize - 0.02f, 0f, fowManager.MapTileSize - 0.02f)/2, Quaternion.identity,LayerMask.GetMask("Wall")))
                    {
                        mapData[i, j] = 1;
                    }
                    else
            ...
            //fowManager.map.InitMap(mapData);
        }
    }
